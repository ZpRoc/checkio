...
# files of a certain type. For example, images with the extension '.jpg' 
# or documents with the extension '.txt', or even files that have the 
# word 'butterfly' in their name. Doing this manually can be time-consuming. 
# 'Matching' or patterns for searching files by a specific mask are 
...
# have txt extension. Here is a small table that shows symbols that can be 
# used in patterns.
...
def unix_match(filename: str, pattern: str) -> bool:
    # your code here
    m = re.match(pattern.replace('.', '\.').replace('*', r'(.*)').replace('?', r'.{1}'), filename)
    return bool(m)
