...
# of algebra in which the values of the variables are true or false, 
...
#   If x is true then the value of x → y is taken to be that of y. But if x is false then 
#   the value of y can be ignored; however the operation must return some truth value and 
#   there are only two choices, so the return value is the one that entails less, namely true.
# - "exclusive" (exclusive or) denoted x ⊕ y and can be described as (x ∨ y)∧ ¬ (x ∧ y). 
#   It excludes the possibility of both x and y. Defined in terms of arithmetic it is addition 
#   mod 2 where 1 + 1 = 0.
# - "equivalence" denoted x ≡ y and can be described as ¬ (x ⊕ y). It's true just when x 
...
# an operation name as described earlier. You should calculate the value 
# and return it as 1 or 0.

# Input: Three arguments. X and Y as 0 or 1. An operation name as a string.
# Output: The result as 1 or 0.
# Precondition: x in (0, 1), y in (0, 1)
