...
# You are given a list of values as set form and a value for which you 
...
# in the ascending order, then to the left of number 9 will be number 7 and 
# to the right - will be number 10. But 10 is closer than 7, which means 
# that the correct answer is 10.
...
def nearest_value(values: set, one: int) -> int:
    # your code here
    sort_val = sorted(list(values))
    diff_val = [abs(i - one) for i in sort_val]

    return sort_val[diff_val.index(min(diff_val))]


def nearest_value_1(values: set, one: int) -> int:
